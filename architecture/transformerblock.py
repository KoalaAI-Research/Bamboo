...
class TransformerBlock(nn.Module):
    def __init__(self, cfg, use_rope=False):
        super().__init__()
        ...
        if cfg["window_size"] is None:
            ...
            self.att = mha.SlidingWindowMultiHeadAttention(
                d_in=cfg["emb_dim"],
                d_out=cfg["emb_dim"],
                context_length=cfg["context_length"],
                num_heads=cfg["n_heads"],
                dropout=cfg["drop_rate"],
                window_size=cfg["window_size"],  # Added window_size to configuration
                qkv_bias=cfg["qkv_bias"]
            )
...
class TransformerBlockWithSlidingWindow(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        self.att = mha.SlidingWindowMultiHeadAttention(
            d_in=cfg["emb_dim"],
